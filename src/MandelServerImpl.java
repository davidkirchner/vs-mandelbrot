...
public class MandelServerImpl extends UnicastRemoteObject implements MandelServer {
  ...
  public MandelServerImpl() throws RemoteException {
    ...
    // LinkedBlockingQueue without a predefined capacity) will cause new tasks
    // to wait in the queue when all corePoolSize threads are busy. Thus, no
    // more than corePoolSize threads will ever be created. (And the value of
    // the maximumPoolSize therefore doesn't have any effect.) This may be
    // appropriate when each task is completely independent of others, so tasks
    // cannot affect each others execution; for example, in a web page server.
    // While this style of queuing can be useful in smoothing out transient
    // bursts of requests, it admits the possibility of unbounded work queue
    // growth when commands continue to arrive on average faster than they can
    // be processed.
    //
  ...
  // { MandelClient mc = iter.next(); try { if (mc.getName().equals(name)) {
  // return false;
  // }
